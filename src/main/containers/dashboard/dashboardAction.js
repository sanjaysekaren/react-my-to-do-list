...
export const AddNewItem = (id, name, date, time) => {
  return async (dispatch, getState) => {
    let mainStateList = getState().dashboardReducers.mainListData;
    let exisitingList = getState().dashboardReducers.mainListData.filter(
      (list) => list.listId === id
    );
    let itemNextId = 1;
...
export const UpdateItem = (listId, itemId, name, date, time) => {
  return async (dispatch, getState) => {
    ...
    mainStateList
      ...
      .items.find((item) => item.itemId === itemId).itemName = name;
    mainStateList
      .find((list) => list.listId === listId)
      .items.find((item) => item.itemId === itemId).createdDate = date;
    mainStateList
      .find((list) => list.listId === listId)
      .items.find((item) => item.itemId === itemId).createdTime = time;
    dispatch(AddNewListToState(mainStateList));
  };
};
...
export const deleteCompleteList = (listId) => {
  return async (dispatch, getState) => {
    ...
    let updatedMainStateList = mainStateList.filter(
...
export const deleteItemsInList = (listId, itemIdList) => {
  return async (dispatch, getState) => {
    ...
    mainStateList.find(
      ...
      .find((list) => list.listId === listId)
      .items.filter((item) => itemIdList.indexOf(item.itemId) === -1);
    dispatch(AddNewListToState(mainStateList));
  };
};
